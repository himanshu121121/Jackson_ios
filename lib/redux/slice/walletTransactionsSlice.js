...
export const fetchWalletTransactions = createAsyncThunk(
  ...
  async (
    { token, limit = 5, force = false, background = false } = {},
    { rejectWithValue, getState, dispatch },
  ) => {
    console.log(
      "[DEBUG-SLICE] fetchWalletTransactions called | force:",
      force,
      "| background:",
      background,
      "| at:",
    ...
    try {
      ...
      if (!force && !background) {
        ...
        const cacheTimestamp =
          state.walletTransactions.transactionsCacheTimestamp;
        ...
        if (
          ...
          if (cacheAge < cacheTTL) {
            ...
            if (cacheAge > cacheTTL * 0.8) {
              setTimeout(() => {
                dispatch(
                  fetchWalletTransactions({ token, limit, background: true }),
                );
              }, 0);
            }
      ...
      const transformedTransactions = response
        ...
        .filter((transaction) => {
          ...
          if (
            ...
            transaction.referenceId.startsWith("SPIN-DC-")
          ) {
            return false;
          }
          return true;
...
export const fetchFullWalletTransactions = createAsyncThunk(
  ...
  async (
    ...
    console.log(
      "[DEBUG-SLICE] fetchFullWalletTransactions called | force:",
      force,
      "| background:",
      background,
      "| at:",
    ...
    try {
      ...
      if (!force && !background && page === 1 && type === "all") {
        const state = getState();
        const cachedFullTransactions =
          state.walletTransactions.fullTransactions;
        const cacheTimestamp =
          state.walletTransactions.fullTransactionsCacheTimestamp;
        const cacheTTL = state.walletTransactions.transactionsCacheTTL;

        if (
          cachedFullTransactions &&
          cachedFullTransactions.length > 0 &&
          cacheTimestamp
        ) {
          const cacheAge = Date.now() - cacheTimestamp;
...
export const fetchWalletScreen = createAsyncThunk(
  ...
  async (
    ...
    console.log(
      "[DEBUG-SLICE] fetchWalletScreen called | force:",
      force,
      "| background:",
      background,
      "| at:",
      ...
      if (!force && !background) {
        ...
        if (cachedWalletScreen && cacheTimestamp) {
          ...
          if (cacheAge < cacheTTL) {
            ...
            if (cacheAge > cacheTTL * 0.8) {
              console.log(
                "[DEBUG-SLICE] fetchWalletScreen: cache 80% expired, triggering background refresh | cacheAge:",
          ...
          // Trigger background refresh immediately
          console.log(
            "[DEBUG-SLICE] fetchWalletScreen: cache STALE, triggering background refresh | cacheAge:",
            Math.round(cacheAge / 1000) + "s",
          );
          setTimeout(() => {
            dispatch(fetchWalletScreen({ token, background: true }));
