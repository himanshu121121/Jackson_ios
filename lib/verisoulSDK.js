...
if (!VERISOUL_API_KEY) {
  console.warn(
    "⚠️ [Verisoul] NEXT_PUBLIC_VERISOUL_API_KEY is not set. Please add it to your .env file.",
  );
} else if (
...
const waitForVerisoul = (maxWaitMs = 10000) => {
  return new Promise((resolve) => {
    if (
      typeof window !== "undefined" &&
      window.Verisoul &&
      typeof window.Verisoul.session === "function"
    ) {
      resolve(true);
      return;
    }
    ...
    const checkInterval = setInterval(() => {
      if (
        ...
        console.warn(
          "⚠️ [Verisoul] SDK not available after waiting",
          maxWaitMs,
          "ms",
        );
        resolve(false);
...
export const initializeVerisoulSDK = async () => {
  if (typeof window === "undefined") {
    ...
    return {
      success: false,
      error: "Server side initialization not supported",
    };
  }

  if (isInitialized && sessionId) {
    console.log(
      "✅ [Verisoul] SDK already initialized with session:",
      sessionId,
    );
  ...
  try {
    ...
    console.log(
      "[Verisoul Debug] After first wait (10s): sdkAvailable =",
      sdkAvailable,
    );
    ...
    if (!sdkAvailable) {
      ...
      console.log(
        "[Verisoul Debug] After retry wait (8s): sdkAvailable =",
        sdkAvailable,
      );
    }
    ...
    if (!sdkAvailable) {
      ...
      if (!isInitialized) {
        console.log(
          "ℹ️ [Verisoul] SDK not ready after wait + retry, using fallback session ID",
        );
      }
      sessionId = generateFallbackSessionId();
      isInitialized = true;
      console.log(
        "[FraudDebug] Verisoul SDK – SDK not available, fallback session_id:",
    ...
    if (window.Verisoul && typeof window.Verisoul.session === "function") {
      try {
        ...
        console.log(
          "[Verisoul Debug] Verisoul.session() result:",
          typeof result,
          result,
        );
        ...
        if (result && result.session_id) {
          ...
          isInitialized = true;
          console.log(
            "✅ [Verisoul] SDK initialized successfully with session:",
            sessionId,
          );
          console.log(
            "[FraudDebug] Verisoul SDK – session_id from SDK:",
            sessionId?.slice?.(0, 24) + "...",
          );
          return { success: true, sessionId };
          ...
          console.log(
            "[FraudDebug] Verisoul SDK – session_id from SDK (string):",
            sessionId?.slice?.(0, 24) + "...",
          );
          return { success: true, sessionId };
        } else {
          console.warn(
            "⚠️ [Verisoul] Unexpected session() response format:",
            result,
          );
          sessionId = generateFallbackSessionId();
          isInitialized = true;
          console.log(
            "[FraudDebug] Verisoul SDK – using fallback session_id:",
            sessionId?.slice?.(0, 24) + "...",
          );
          return { success: true, sessionId, fallback: true };
        }
        ...
        console.log(
          "[Verisoul Debug] session() error details:",
      ...
      logVerisoulState(
        "SDK check failed (Verisoul or session not ready) – using fallback",
      );
      sessionId = generateFallbackSessionId();
      isInitialized = true;
      return { success: true, sessionId, fallback: true };
